...
func initializeTables(db *gorm.DB) {
	err := db.Set("gorm:table_options", "ENGINE=InnoDB CHARSET=utf8mb4").AutoMigrate(
		...
		&goodsDetailsModels.GoodsDetail{},
		&usersModels.User{},
	)
	if err != nil {
		global.App.Log.Info("migrate table failed", err.Error())
	}
}
...
func initializeAdminUser(db *gorm.DB) {
	...
	_ = db.FirstOrCreate(&user, usersModels.User{
		Username:    "admin",
		Password:    "$2a$10$RlfVKuZ.eKH3HV1zgl167u/sTGOiPRvRKlSeoNYKGDAoTix9CIFHq",
